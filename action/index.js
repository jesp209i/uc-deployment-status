...
async function run() {
  ...
  const timeoutSeconds = (0, import_core.getInput)("timeout-seconds", { required: true });
  let interval;
  let timeout;
  let currentRun = 1;
  let messageCursor = 0;
...
run();
async function getStatusFromApi(baseUrl, apiKey, projectAlias, deploymentId) {
...
function apiRejectedResponse(rejected) {
  if (typeof rejected === "string") {
    (0, import_core.info)(`Error while calling ApiClient: ${rejected}`);
  }
  if (typeof rejected === "object") {
    const problemDetails = rejected;
    (0, import_core.info)(`Api returned ${problemDetails.statusCode} details: ${problemDetails.details}`);
  }
  (0, import_core.setFailed)("Error happened during polling for Deployment Status");
}
